slb <- function(pred,med,out,time,group,id,boot=1000){
  UseMethod("slb")
}

slb.default <- function(pred,med,out,time,group,id,boot=1000){
...
plot.slb <- function(x,main="",col="lightyellow",border="gray",in.color=T,...){
  if(in.color==T){
...
qq.slb <- function(x,main="",...){
  qqnorm(x$boot.results$ind.eff)
  qqline(x$boot.results$ind.eff)
}

## drawfig.slb <- function(x,iv="IV",med="MED",dv="DV"){
##   library(RGraphviz)
